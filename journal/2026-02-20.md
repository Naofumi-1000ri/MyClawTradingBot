...
## Coder Task: 全エージェント失敗追跡ロジック強化 (部分失敗も計上) [2026-02-20]

- **問題**: 既存の `_run_rubber_wall()` 内の失敗判定が `not btc_5m and not eth_5m and not sol_5m` （AND条件）だった。
  このため、1〜2銘柄のキャンドルデータが欠損しても「成功」扱いでカウントがリセットされ、障害がサイレントに継続するリスクがあった。
  また、`_run_rubber_wall()` の戻り値が `None` のため `main()` 側で正確な失敗判定ができていなかった。
- **変更ファイル**: `src/brain/brain_consensus.py`
- **対応内容**:
  1. `_run_rubber_wall()` の戻り値を `None → bool` に変更 (True=全銘柄スキャン失敗, False=少なくとも1銘柄完了)
  2. `scan_failed_count` 変数を追加: 各銘柄でキャンドルデータなし時に `+1`
  3. 全3銘柄が失敗した時のみ `True` を返すよう判定 (`scan_failed_count >= 3`)
  4. 部分失敗 (1〜2銘柄欠損) は `logger.warning` でのみ記録し、失敗カウントには加算しない
  5. `main()` 内で `all_scan_failed: bool = call_with_retry(_run_rubber_wall, ...)` として戻り値を受け取り、`_track_agent_failure(failed=all_scan_failed)` を呼び出す
  6. ゴム戦略クラッシュ (`RetryExhausted` / `Exception`) 時は従来通り `_track_agent_failure(failed=True)`
  7. ログメッセージを拡張: `=== Rubber Complete: ... scan_failed=N/3 ===` で欠損銘柄数を表示
- **失敗カウント対象** (変更なし): 全3銘柄のキャンドルデータが全滅した場合 + コンテキスト構築失敗 + ゴム戦略クラッシュ
- **アラート動作** (変更なし): 3連続失敗で `kill_switch.json warning=True` + journal CRITICAL + Telegram通知
...
## Coder Task: enter_safe_hold() 絶対パス修正 [2026-02-20]

- **問題**: `src/utils/retry.py` の `enter_safe_hold()` が相対パス (`Path("signals/signals.json")`, `Path("state/kill_switch.json")`) を使用していた。カレントディレクトリが `/home/claw/myClaw` 以外の場合にファイルが正しい場所に書かれないリスクがある。
- **変更ファイル**: `src/utils/retry.py`
- **対応内容**:
  - `from src.utils.config_loader import get_project_root` をインポート
  - `ROOT = get_project_root()` でプロジェクトルートを取得
  - `signals/signals.json` → `ROOT / "signals" / "signals.json"` に変更
  - `state/kill_switch.json` → `ROOT / "state" / "kill_switch.json"` に変更
- **構文チェック**: py_compile にてエラーなし ✓

## Coder Task: ETH戦略精度向上 - 4Hフィルター強化・Pattern B SL最低距離改善 [2026-02-20]

### 背景分析

**対象のSLヒット事例**: 2026-02-20 10:37 UTC (修正前コードによるもの)
- entry=1966.8, exit=1963.1, SL距離0.19%, 保有5分
- **コミット71ff420 (12:45 UTC) 以前に発生** → 既に修正済みコードのSLヒットではない

**トレード履歴全体分析** (ETH 19件クローズ):
- LONG: 6勝/8敗 = 43%勝率 (不採算)
- SHORT: 3勝/2敗 = 60%勝率 (採算)
- 合計PnL: +$0.06 (ほぼゼロ)
- 問題: 4Hダウントレンド中にLONGが多発。4H下位40%でも不十分なケースがある。
...
| 項目 | 変更前 | 変更後 | 理由 |
|------|--------|--------|------|
| `reversal_h4_filter_pct` | 40% | **50%** | 4H中位以下でのLONG抑制を強化。ダウントレンド中の逆張り回避 |
| Pattern B SL最低距離 | 0.1% (`if sl_dist < 0.001`) | **0.2%** (`max(candle_sl, entry*1.002)`) | small candle highで過接近SLとなるケース防止 |
| docstring `SL = max(...)` | max(誤記) | **min(正しい)** | コードは正しいが文書が誤っていた修正 |

### 変更しなかったもの (維持理由)

- `reversal_threshold`: 7.0を維持 (8.0への引き上げはデータ不足のため時期尚早)
- `reversal_sl_min_dist`: 0.25%維持 (前回修正で既に適切)
- `reversal_tp_pct`: 0.5%維持 (R:R 2.0の維持)

### 4Hフィルター50%の効果

4H中位 (50%) 以下ではPattern A LONG を完全抑制:
- ダウントレンドの確認精度向上
- トレード頻度は下がるが質が向上
- 4H pos=30-50%のグレーゾーントレードを排除

### 構文チェック
- `py_compile`: エラーなし ✓

## Coder Task: Rubber戦略パフォーマンス追跡・分析機能追加 [2026-02-20]

### 目的

戦略の有効性を定量的に評価するために、シグナルログと損益結果を紐付けた
パフォーマンス分析・レポート機能を実装する。特にpenetration zoneでのエントリーと
vol_ratioの関連性を検証できるようにする。

### 変更ファイル

- `src/monitor/performance_tracker.py` (新規作成)
- `src/monitor/monitor.py` (run_analysisの呼び出し追加)

### performance_tracker.py の機能

**入力ファイル:**
- `state/rubber_signal_log.json` - brain_consensus._log_rubber_signal() が記録するシグナルログ
- `state/trade_history.json` - state_manager.record_trade() が記録するトレード履歴

**結合方式:** 同シンボル + シグナルtimestamp <= トレードclosed_at + 2時間以内
(エントリーからクローズまでのタイムラグを2時間窓でカバー)

**集計軸:**
1. **ゾーン別** (`penetration` / `upper_range` / `deep_reversal` / `reversal` / `momentum`)
2. **vol_ratio帯別** (`<3x` / `3-5x` / `5-7x` / `7-10x` / `10x+`)
3. **シンボル別** (BTC / ETH / SOL)
4. **パターン別** (`wall_penetration` / `A_reversal` / `B_momentum` 等)

**penetration zone 有効性分析:**
- penetration zone vs non-penetration の勝率・PF・avg_pnl 比較
- penetration zone 内での vol_ratio強度帯別の追加集計

**出力ファイル:** `state/performance_report.json`

### monitor.py の変更

`run_monitor()` 内のアラート送信前に `run_analysis(save_report=True)` を呼び出し追加。
12サイクルごと (約1時間ごと) の定期分析レポートが自動実行される。

### 初回実行結果

現時点でのシンボル別分析 (rubber_signal_logなしの全トレード集計):
- BTC: 17件 勝率29.4% avg_pnl=+0.0338 **PF=8.03** ← 優秀
- ETH: 38件 勝率23.7% avg_pnl=+0.0016 **PF=1.04** ← 辛うじてプラス
- SOL: 53件 勝率13.2% avg_pnl=-0.0395 **PF=0.12** ← 要改善

rubber_signal_log は本日 (2026-02-20) から記録開始のため現在4件。
今後蓄積によりゾーン別・vol_ratio帯別の詳細分析が可能になる。

### 構文チェック

- `src/monitor/performance_tracker.py`: py_compile エラーなし ✓
- `src/monitor/monitor.py`: py_compile エラーなし ✓

## Coder Task: サイレントフォールバック防止・自動アラート強化 [2026-02-20]

### 問題: サイレントフォールバック

既存の実装では以下の失敗がサイレントにフォールバックされていた:
1. `data_collector.py` でキャンドルデータ取得失敗 → 前回データ使用 (ログのみ)
2. `data_health_check.py` でスコア低下 → `requests.json` 書き込みのみ (Telegram通知なし)
3. `monitor.py` でデータ品質継続劣化の検知なし

### 変更ファイル

**`src/collector/data_health_check.py`** (untracked → 今回コミット):
- `_should_send_alert(state_dir, alert_type, cooldown_seconds)`: 30分クールダウン管理
- `_send_health_alert(result, settings)`: データ品質劣化時にTelegram通知
  - score < 80 または execution_mode=close_only → WARNING通知
  - recommend_kill_switch (score < 60) → CRITICAL通知
  - アラート状態は `state/data_health_alert_state.json` で管理

**`src/collector/data_collector.py`**:
- `fallback_events: list[str]` を追加してフォールバック使用状況を追跡
- 各シンボルの mid_price / 各時間足candles / orderbook / funding_rate のフォールバック使用時にイベント記録
- 5m_candles または mid_price フォールバック発生時にTelegram通知 (30分クールダウン)
  - `state/collector_fallback_state.json` でクールダウン管理

**`src/monitor/monitor.py`**:
- `data_health_summary.json` の `consecutive_low_score` 監視を追加 (セクション5)
- 3サイクル以上連続スコア低下 (= 15分以上データ品質劣化継続) でアラートリストに追加
- → 既存のアラート送信ロジック (セクション6) でTelegram一括送信される

### アーキテクチャ

```
データ収集フェーズ:
  data_collector.py → fallback_events追跡 → Telegram (5m_candles/mid失敗時, 30分CD)
  data_health_check.py → スコア計算 → Telegram (score<80時, 30分CD)

モニタリングフェーズ:
  monitor.py → consecutive_low_score >= 3 → alerts[] → Telegram一括送信
```

### 既存機能との関係

- `brain_consensus.py` の `_track_agent_failure()` は変更なし
- `src/utils/retry.py` の `enter_safe_hold()` は変更なし
- 変更はすべて「通知」レイヤーの強化のみ。リスク判断・執行ロジック変更なし。

### 構文チェック

- `src/collector/data_health_check.py`: py_compile エラーなし ✓
- `src/collector/data_collector.py`: py_compile エラーなし ✓
- `src/monitor/monitor.py`: py_compile エラーなし ✓
