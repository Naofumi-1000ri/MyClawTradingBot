...
Pattern C (quiet_long):  スパイクなし + 低出来高 + GOLDEN + 4H低位 → LONG
  ...
  - 条件: EMA9 > EMA21 (GOLDEN), 4H pos < 45% (中低位), 直近5本/100本出来高比 < 0.60
  ...
  - 2026-02-21 h4_max_pct再調整: 50%→45%
    理由: 直近28時間BTでpos=43-50%ゾーンはtimeoutが多く(5件中3件)EV低下
    pos<45%では30日BT WR=68.2%の品質を維持しつつ無駄なtimeout削減
...
_DEFAULT_CONFIG = {
    ...
    "quiet_long_h4_max_pct": 45,         # 4H pos < 45% (旧50%→45%: 40-50%ゾーンはtimeout多くEV低下)
